...
const initialState:articleState={
  articleData: [
    ...
    {
      image_url: 'https://effybiz-devops.s3.ap-south-1.amazonaws.com/bde86aee-6d42-454e-8be7-3839cf6b3049_frame_35.jpg'
    },
    {
...
const articleSlice = createSlice({
  ...
  reducers: {
    ...
    highlightFindText: (state) => {
      if (state.findText) {

        state.articleData = state.articleData.map(article => {
          if (article.text) {
            const finalArr=[]
            for(const text of article.text.split(' ')){
              if(text.toLowerCase()===state.findText.toLowerCase()){
                finalArr.push(`<mark>${text}</mark>`)
              }else{
                finalArr.push(text)
              }
            }
            article.text=finalArr.join(' ')
          }
          return article;
        });
      }
...
export const {setArticleData,setFindText,setReplaceText,highlightFindText}=articleSlice.actions

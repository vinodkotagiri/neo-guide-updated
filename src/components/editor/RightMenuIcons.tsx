...
function MenuIconButton({label, icon,setRightActiveArea,val,rightActiveArea }: { label:string,icon: React.ReactNode ,setRightActiveArea:any,val:number,rightActiveArea:number}) {
 const {locked}=useAppSelector(state=>state.video)
  return (
    <button disabled={locked} style={rightActiveArea==val?{backgroundColor:'#02BC7D',color:'#fff'}:{}} className='btn btn-ghost rounded-none outline-none shadow-none h-[64px] flex flex-col w-full justify-center items-center border-t-[1px] border-slate-600 p-2' onClick={()=>setRightActiveArea(val)}>
      {icon}
     <small>{label}</small> 
    </button>
  );

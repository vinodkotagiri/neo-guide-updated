...
const DraggableResizableSlice = ({playerRef,wrapperRef,layerRef,sliceRef,label,shapeType,shape,color='#02BC7D'}) => {
  ...
  return (
    <div
      className="h-full absolute cursor-grab user-select-none flex items-center justify-center capitalize text-xs"
      ref={sliceRef}
      style={{left:sliceStyle.left,width:sliceStyle.width,backgroundColor:color}}
      onMouseDown={handleMouseDown}
      onClick={onShapeClick}
    >
      <p>{label}&nbsp;<span className='font-semibold'>{(sliceStyle.width/8)}s</span></p>
